...
open class JXSegmentedTitleAttributeDataSource: JXSegmentedBaseDataSource {
    ...
    open func widthForTitle(_ title: NSAttributedString?, selectedTitle: NSAttributedString?) -> CGFloat {
        ...
        if widthForTitleClosure != nil {
            ...
            let textWidth = text.boundingRect(with: CGSize(width: CGFloat.infinity, height: CGFloat.infinity), options: NSStringDrawingOptions(rawValue: NSStringDrawingOptions.usesLineFragmentOrigin.rawValue | NSStringDrawingOptions.usesFontLeading.rawValue), context: nil).size.width
    ...
    open override func preferredSegmentedView(_: JXSegmentedView, widthForItemAt index: Int) -> CGFloat {
        var width: CGFloat = 0
